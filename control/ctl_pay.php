...
class ctl_pay {
	...
	public $cache_prefix = 'zb.03ky.com';
	...
	public function go() {//跳转支付
		...
		if($type==2){//支付宝支付
          ...
          if(preg_match('/(blackberry|configuration\/cldc|hp |hp-|htc |htc_|htc-|iemobile|kindle|midp|mmp|motorola|mobile|nokia|opera mini|opera |Googlebot-Mobile|YahooSeeker\/M1A1-R2D2|android|iphone|ipod|mobi|palm|palmos|pocket|portalmmm|ppc;|smartphone|sonyericsson|sqh|spv|symbian|treo|up.browser|up.link|vodafone|windows ce|xda |xda_)/i', $_SERVER['HTTP_USER_AGENT'])){
					 $gourl = 'https://sm.03ky.com/payment/alipay_wap/wappay/pay.php?'.http_build_query($orders);
				}else{
					 $gourl = 'https://sm.03ky.com/payment/alipay_pc/wappay/pay.php?'.http_build_query($orders);
				}
            ...
            if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false ) {//微信内
					header('Location: http://cesuan.zhan2345.com/getcode.php?auk=demo3&oid='.$oid);
				...
				if(preg_match('/(blackberry|configuration\/cldc|hp |hp-|htc |htc_|htc-|iemobile|kindle|midp|mmp|motorola|mobile|nokia|opera mini|opera |Googlebot-Mobile|YahooSeeker\/M1A1-R2D2|android|iphone|ipod|mobi|palm|palmos|pocket|portalmmm|ppc;|smartphone|sonyericsson|sqh|spv|symbian|treo|up.browser|up.link|vodafone|windows ce|xda |xda_)/i', $_SERVER['HTTP_USER_AGENT'])){
					...
					header('Location: http://cesuan.zhan2345.com/payment/Wxpay_gz/example/native.php?'.$httpdata);
					die;
				}
	...
	public function pay_go() {//3方支付跳转支付
		...
		if($type=="alipay"){//支付宝支付
			...
			$gourl = 'https://sm.03ky.com/SDK/epayapi.php?'.http_build_query($orders);
            mod_order::up_order(array('paytype'=>2,'paytime'=>date('Y-m-d G:i:s',time()))," `oid`='".$oid."'");
        }else{
    ...
    public function paypal_go() {//paypal跳转支付
		$oid = req::item('oid');
			...
			$gourl = 'https://sm.03ky.com/payment/paypal/paypal.php?'.http_build_query($orders);
	...
	public function notify() {
        ...
        if($orders){
            if (1 == 1) {//查看商户号以及订单状态
                ...
                header("Location:http://cesuan.zhan2345.com/?ac=".$ac."&oid=".$out_trade_no."&token=".base64_encode(md5($out_trade_no)));
                exit;
            }
        }
    }
	public function notify_yzf() {
		...
		$ac=mod_order::typetochannel($orders['type']);
		header("Location:https://sm.03ky.com/?ac=".$ac."&oid=".$out_trade_no."&token=".base64_encode(md5($out_trade_no)));
		exit;
    }
	...
	public function scanquery(){
		...
		if($row['status']=='1'){
			$return['status'] = true;
			$ac=mod_order::typetochannel($row['type']);
			$return['url'] = "http://cesuan.zhan2345.com/?ac=".$ac."&oid=".$oid."&token=".base64_encode(md5($oid));
		}else{
			$return['status'] = false;
			$return['url'] = '';
		}
    ...
    //  $orders = array('WIDout_trade_no'=>$row['oid'],'WIDsubject'=>$row['des'],'WIDtotal_amount'=>1,'WIDbody'=>$row['des']);
	  ...
	  //echo 'http://sm.03ky.com/payment/Wxpay/example/h5api.php?'.$httpdata;die;
   	  ...
   	  header('Location: https://sm.03ky.com/payment/Wxpay_gz/example/h5api.php?'.$httpdata);
	  die; 
      send('https://sm.03ky.com/payment/Wxpay_gz/example/h5api.php',$orders);
      die;
    ...
    public function wxjsapi(){
		...
		$ac=mod_order::typetochannel($row['type']);
		$row['url'] = "http://cesuan.zhan2345.com/?ac=".$ac."&oid=".$oid."&token=".base64_encode(md5($oid));
		$row['oid'] = $oid;
        $sub_openid = $openid;
        ...
        $return = send('http://cesuan.zhan2345.com/payment/Wxpay_gz/example/jsapi.php',$data);
