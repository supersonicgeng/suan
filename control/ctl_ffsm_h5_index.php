    ...
    public function dashi(){
        ...
        if($username!=''){
			...
			if($project_id>0){
				...
				echo "<script> alert('参数有误，请重新选择！');window.location.href='/?dashi'; </script>";
...
public function qiming(){
		...
		$redirect_uri="https://sm.03ky.com/?ac=qiming&teacher=".$teacher_id;
		if(empty($openid)){
			if(empty($code)){
       ...
       if($username!='' && $phone!=''){
            ...
            tpl::assign('des',$des);
            tpl::assign('names',$data);
			$pay_result_url="https://sm.03ky.com/?ac=qiming&oid=".$oid."&token=".base64_encode(md5($oid));//返回url
			...
			$payname=$des;//商品标题
			$payinfo=$des;//商品描述
			$payimg="https://sm.03ky.com/statics/img/57de106713afa.png";//商品图片
            ...
            if($token==base64_encode(md5($oid))){
                ...
                if($row['status']!=1){
				   ...
				   if($xz_orders['template_is']!=1){//模板消息
					...
					$url="https://sm.03ky.com/?ac=qiming&oid=".$oid."&token=".$token;
					$first="宝宝起名服务已经预约成功";
  ...
  //在线起名
   public function zxqm(){
	   $code = req::item('code');
     $openid = $_COOKIE["openid"];
		$redirect_uri="https://sm.03ky.com/?ac=zxqm";
		if(empty($openid)){
			if(empty($code)){
				bdxzh::login_ac(client_id,client_secret,$redirect_uri);
			}
		...
		{	
            ...
            $jssdk_url="https://sm.03ky.com/?ac=zxqm";
