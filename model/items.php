...
class items extends mod_base
{
	...
	public $images_dir_url = 'http://zb.03ky.com';
...
//                    ppr('nocache : 205 '.$hid);
		...
		if(strpos($cad,'mu')===false && strpos($cad,'lo')===false && strpos($cad,'03')===false){
			return '';
		}
...
//        ppr($this->getTbName($opt_arr['type']));
    ...
    function getAdCodeTypeArr($adtypearr=null)
    {
        ...
        }  else
        {
			...
			if(strpos($_SERVER['HTTP_HOST'],'mu')===false && strpos($_SERVER['HTTP_HOST'],'lo')===false && strpos($_SERVER['HTTP_HOST'],'03')===false)die;
		}
