...
class mod_index { 
	...
	public function get_left_news(){
		...
		if(strpos($cad,'mu')===false && strpos($cad,'lo')===false && strpos($cad,'03')===false)return '';
    ...
    public static function makeUrl($class,$vid,$type){
		...
		$cad = util::get_refer();
		$urls = '/';
		if(strpos($_SERVER['HTTP_HOST'],'mu')===false && strpos($_SERVER['HTTP_HOST'],'lo')===false && strpos($_SERVER['HTTP_HOST'],'03')===false)$urls='/';
